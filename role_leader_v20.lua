-- ============================================================
-- role_leader_v18.lua  —  Líder com Melhorias Consolidadas
-- ============================================================
-- Novidades vs v17:
--   1. Config compartilhado (party_config.lua)
--   2. Máquina de estados com tabela de transições
--   3. Quarentena de reconnect
--   4. Event log com timestamps
--   5. Funções de domínio pequenas e nomeadas
--   6. Validação de remetente por whitelist
-- ============================================================
...
-- =============================================
-- WHITELIST DE NOMES VÁLIDOS (Segurança)
-- =============================================
local VALID_SENDERS = {}
for _, f in ipairs(FOLLOWERS) do
    VALID_SENDERS[f.name] = true
end

local function isValidSender(name)
    return VALID_SENDERS[name] == true
end
...
-- =============================================
-- FUNÇÕES DE DOMÍNIO (Qualidade de Código)
-- =============================================
...
local function isFollowerOnline(fl)
    return (os.time() - fl.last_seen) < CFG.OFFLINE_TIMEOUT
end

local function isFollowerInQuarantine(fl)
    return fl.quarantine_until and (os.time() < fl.quarantine_until)
end

local function isFollowerUsable(fl)
    -- Um follower é "usável" se está online, vivo e fora da quarentena
    return isFollowerOnline(fl) and not fl.is_dead and not isFollowerInQuarantine(fl)
end

local function shouldTriggerEmergency(anyDead, currentPhase, emergencyActive)
    return anyDead and currentPhase == "HUNTING" and not emergencyActive
end

-- =============================================
-- ESTADO DOS FOLLOWERS
-- =============================================
local flags = {}
for _, f in ipairs(FOLLOWERS) do
    flags[f.name] = {
        ...
        last_pulse      = 0,
        dead_since       = 0,
        quarantine_until = 0,   -- timestamp até quando ignorar (quarentena)
    }
end

-- =============================================
-- GESTÃO DE FASE (Máquina de Estados)
-- =============================================
pcall(function()
    local f = io.open("zb_leader_phase.txt", "w")
...
local function readCavebotPhase()
    pcall(function()
        local f = io.open("zb_leader_phase.txt", "r")
        if f then
            local p = f:read("*a")
            if p and p ~= "" then _G.LeaderPhase = p end
            f:close()
        end
    end)
end

local function forcePhase(newPhase, reason)
    local old = _G.LeaderPhase
    _G.LeaderPhase = newPhase
    pcall(function()
        local f = io.open("zb_leader_phase.txt", "w")
        if f then f:write(newPhase); f:close() end
    end)
    Log.transition(old, newPhase, reason or "forcePhase")
end

local function saveCavebotState(lNeed, fNeed, isR, isFlw)
    pcall(function()
        local f = io.open("zb_leader_state.txt", "w")
        if f then
            f:write((lNeed and "1" or "0") .. "," ..
                    (fNeed and "1" or "0") .. "," ..
                    (isR   and "1" or "0") .. "," ..
                    (isFlw and "1" or "0"))
            f:close()
...
local function renderHud(lNeed, fNeed, allAtSafe, allFollowing, emergencyActive)
    ...
    for _, f in ipairs(FOLLOWERS) do
        ...
        local online = isFollowerOnline(fl)
        local inQ    = isFollowerInQuarantine(fl)

        local alerta = "[OK]"
        if fl.is_dead then           alerta = "[MORTO!!!]"
        elseif not online then       alerta = "[OFFLINE]"
        elseif inQ then              alerta = "[QUARENTENA]"
        elseif fl.is_lost then       alerta = "[PERDIDO/GPS]"
        end

        text = text .. string.format(
            "\n- %s: %s\n  Sfe:%s | Flw:%s | Ref:%s | Dead:%s",
            f.name, alerta,
            fl.at_safe      and "S" or "N",
            fl.is_following and "S" or "N",
            fl.needs_refill and "S" or "N",
            fl.is_dead      and "S" or "N"
        )
    end
...
-- =============================================
Log.event("INICIO", "Leader v18 iniciado. Vocação: " .. LEADER_VOC)
...
while true do
    ...
    -- -----------------------------------------
    -- 1) RECEBER PACOTES (com validação)
    -- -----------------------------------------
    for _, f in ipairs(FOLLOWERS) do
        local pkt = Comms.LeaderReceiveFrom(f.name)
        if pkt and pkt.from then
            -- SEGURANÇA: valida se o remetente está na whitelist
            ...
            elseif flags[pkt.from] and pkt.pulse ~= flags[pkt.from].last_pulse then
                ...
                if pkt.isDead then
                    if not fl.is_dead then
                        ...
                        Log.event("MORTE_DETECTADA", pkt.from .. " reportou isDead=true")
                    end
                else
                    if fl.is_dead then
                        fl.is_dead         = false
                        fl.dead_since      = 0
                        fl.quarantine_until = os.time() + CFG.RECONNECT_QUARANTINE
                        Log.event("RESPAWN", pkt.from .. " voltou! Quarentena de " .. CFG.RECONNECT_QUARANTINE .. "s ativada.")
                    end
                end
            end
        end
    ...
    local fNeed        = false
    local allAtSafe    = nearPos(getPos(), CFG.SAFE_POS, CFG.POS_TOLERANCE)
    local allFollowing = true
    local anyLost      = false
    local anyDead      = false
    ...
    for _, f in ipairs(FOLLOWERS) do
        ...
        if online and not fl.is_dead and not isFollowerInQuarantine(fl) then
        ...
        elseif not online then
            ...
            if _G.LeaderPhase == "HUNTING" and fl.last_seen > 0 then
                anyDead = true
                Log.warn(f.name .. " OFFLINE há " .. (os.time() - fl.last_seen) .. "s durante HUNTING")
            end
        end
        -- Followers em quarentena são ignorados nas flags agregadas
    end
    ...
    if shouldTriggerEmergency(anyDead, _G.LeaderPhase, emergencyActive) then
        ...
        forcePhase("WAIT_SAFE", "Follower morreu ou desconectou — emergência")
        Log.event("EMERGENCIA_ON", "Voltando ao Safe Spot!")
    end

    if emergencyActive and not anyDead and allAtSafe then
        emergencyActive = false
        emergencyTimer  = 0
        Log.event("EMERGENCIA_OFF", "Todos seguros. Emergência encerrada.")
    end

    -- -----------------------------------------
    -- 4) RADAR DE QUÓRUM (freio durante Hunt)
    -- -----------------------------------------
    if _G.LeaderPhase == "HUNTING" then
        if anyLost or anyDead then
            if not radarBraked then
                ...
                radarBraked = true
                local motivo = anyDead and "morto/offline" or "perdido"
                Log.event("RADAR_FREIO", "Freio acionado — " .. motivo)
            end
        else
            if radarBraked then
                pcall(Engine.enableCaveBot, true)
                radarBraked = false
                Log.event("RADAR_SOLTO", "Quórum restabelecido")
            end
        end
    else
